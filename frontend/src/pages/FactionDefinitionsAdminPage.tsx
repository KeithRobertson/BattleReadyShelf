import {Alert, Button, Group, Loader, Modal, Select, Stack, Table, Text, TextInput, Title, ActionIcon} from "@mantine/core";
...
import {createFaction, getFactions, deleteFaction} from "../generated";
...
export default function FactionDefinitionsAdminPage() {
    ...
    const [selectedFactionDefinitionIds, setSelectedFactionDefinitionIds] = useState<Set<string>>(new Set());
        ...
        <Stack gap="md">
            <Group justify="space-between">
                <div>
                    ...
                    <Text c="dimmed">
                        Create and edit factions available to users.
                        WIP screen - the external_id needs explained, and Parent Faction
                    </Text>
                </div>
                {isAdmin && (
                    <Group>
                        ...
                        <Group justify="space-between" mb="xs">
                            ...
                            {factions.size > 0 && (
                                <Button
                                    ...
                                    loading={deletingSelectedFactionDefinitions}
                                    onClick={handleDeleteSelectedFactionDefinitions}
                                >
                                    Delete selected ({selectedFactionDefinitionIds.size})
                                </Button>
                            )}
                        </Group>
                        {factions.length === 0 ? (
                                ...
                                <Table>
                                    ...
                                    <Table.Tbody>
                                        {factions.map((faction) => {
                                            ...
                                            return <Table.Tr key={faction.id}>
                                                <Table.Td>
                                                    {faction.name}
                                                </Table.Td>
                                                <Table.Td>
                ...
                <form onSubmit={handleCreateNew}>
                    <Stack>
                        <TextInput label="Name" value={newName} onChange={(e) => setNewName(e.currentTarget.value)}
                                   required/>
                        <TextInput label="External Faction Id" value={newExternalId}
